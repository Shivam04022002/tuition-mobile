...
function iconForType(type: string): { name: string; color: string } {
  switch (type) {
    case 'TEACHER_APPLIED':         return { name: 'person-add',       color: colors.secondary };
    case 'DEMO_SCHEDULED':
    case 'DEMO_CONFIRMED':          return { name: 'event',             color: colors.info };
    case 'PAYMENT_SUCCESS':         return { name: 'check-circle',      color: colors.success };
    case 'REFUND_APPROVED':         return { name: 'monetization-on',   color: colors.success };
    case 'REFUND_REJECTED':         return { name: 'cancel',            color: colors.error };
    case 'NEW_LEAD_MATCH':          return { name: 'star',              color: colors.accent };
    case 'APPLICATION_SHORTLISTED': return { name: 'bookmark',          color: colors.secondary };
    case 'APPLICATION_ACCEPTED':    return { name: 'thumb-up',          color: colors.success };
    case 'APPLICATION_REJECTED':    return { name: 'thumb-down',        color: colors.error };
    case 'LEAD_UNLOCK_SUCCESS':     return { name: 'lock-open',         color: colors.primary };
    case 'REFUND_REQUEST':          return { name: 'assignment-return', color: colors.accent };
    case 'TEACHER_REGISTRATION':    return { name: 'school',            color: colors.info };
    case 'IMPORT_COMPLETED':        return { name: 'upload-file',       color: colors.info };
      ...
      >
        ...
        <View style={styles.content}>
          <View style={styles.titleRow}>
            <Text
              style={[styles.title, { fontWeight: unread ? '700' : '500' }]}
              numberOfLines={1}
            >
              {item.title}
            </Text>
